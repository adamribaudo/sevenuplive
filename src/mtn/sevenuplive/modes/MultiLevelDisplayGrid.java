/*
	Copyright 2011 Adam Ribaldo, Chris Lloyd
		...
		for (int x = 0; x<grid_width-1;x++)
		{
			for (int y = 0; y<grid_height;y++)
			{
				...
				case 0:
					...
					break;
				case 3:  
					if(force || monome.getValue(x+start_column, y+start_row) != HIGH_INTENSITY)
						monome.setValue(x+start_column, y+start_row, HIGH_INTENSITY);
					break;

				case 2:   
					if(force || monome.getValue(x+start_column, y+start_row) != MED_INTENSITY)
						monome.setValue(x+start_column, y+start_row, MED_INTENSITY);
					break;
				case 1:
					if(force || frmCount % SLOWBLINKFRAME == 0) 
					{
						monome.setValue(x+start_column, y+start_row, monome.isLit(x+start_column, y+start_row) ? 0 : HIGH_INTENSITY);
					}
					break;
				}      
			}
	...
	protected void drawNav(boolean force) {
		...
		for (int y = 0; y<grid_height;y++)
		{
			...
			case 0:
				...
				break;
			case 3:  
				if(force || monome.getValue(NAVCOL+start_column, y+start_row) != HIGH_INTENSITY)
					monome.setValue(NAVCOL+start_column, y+start_row, HIGH_INTENSITY);
				break;

			case 2:   
				if(force || frmCount % FASTBLINKFRAME == 0) 
				{
					monome.setValue(NAVCOL+start_column, y+start_row, monome.isLit(NAVCOL+start_column, y+start_row) ? 0 : HIGH_INTENSITY);
				}
				break;
			case 1:
				if(force || frmCount % SLOWBLINKFRAME == 0) 
